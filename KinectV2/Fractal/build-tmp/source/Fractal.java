...
//  theta1 = -acos(yval/sqrt(sq(xval)+sq(yval)));
//  }
  theta1 = PVector.angleBetween(new PVector(convertedRightHand.x-convertedTorso.x,convertedRightHand.y-convertedTorso.y,0.0f),new PVector(0,convertedTorso.y-height,0.0f));
  if (xval<0){
    theta1*= -1;
  }
  ...
  theta2 = PVector.angleBetween(new PVector(convertedLeftHand.x-convertedTorso.x,convertedLeftHand.y-convertedTorso.y,0.0f),new PVector(0,convertedTorso.y-height,0.0f));
  if (xval<0){
    theta2*= -1;
  }
...
public void branch(float len) {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "Fractal" };
    if (passedArgs != null) {
